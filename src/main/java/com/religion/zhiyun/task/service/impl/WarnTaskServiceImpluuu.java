...
@Slf4j
@Service
public class WarnTaskServiceImpluuu implements WarnTaskService {
    @Autowired
    ...
    public Object deployment() {
        ...
        builder.addClasspathResource("process/bp2.bpmn");
        String id = builder.deploy().getId();
        repositoryService.setDeploymentKey(id,"warnEventTask");
        log.info("流程id："+id);
        return id;
    }
    ...
    public Object launch() {
        ...
        variables.put("assignee1", userId);
        variables.put("assignee2", ofcId);
        String identity = sysUserEntity.getIdentity();
        int type=0;
        if("10000007".equals(identity)){
            type=1;
        }else if("10000006".equals(identity)){
            type=2;
...
/**start**//*
        ...
        ProcessInstance processInstance =runtimeService.startProcessInstanceByKey("eventTask1",variables);
...
/**end**//*
    ...
    public Object report() {
        ...
        if(!ObjectUtils.isEmpty(T)) {
            for (Task item : T) {
                ...
                variables.put("outcome", "go");
                variables.put("overcome", "");
                variables.put("condition", "go1");
    ...
    public Object approve() {
        ...
        if(!ObjectUtils.isEmpty(T)) {
            for (Task item : T) {
                Map<String, Object> variables = new HashMap<>();
                variables.put("inputUser", ofcId);
                variables.put("assigneeUserId",ofcId);
                variables.put("isSuccess", true);
                variables.put("overcome", "end");
                variables.put("outcome", "");
                variables.put("condition", "go2");
                String identity = sysUserEntity.getIdentity();
                if("10000006".equals(identity)){
                    variables.put("assignee3",ofcId);
                }else if("10000005".equals(identity)){
